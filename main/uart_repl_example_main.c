...
static int led_console(int argc, char **argv)
{
    ...
    int led_no = atoi(argv[1]); // แปลงพารามิเตอร์ตัวที่ 2  (หมายเลข LED) เป็นตัวเลข
                                // ข้อสังเกตุ ต้องรับมือในกรณี user ป้อนอักขระที่ไม่ใช่ตัวเลข!!!
    printf("LED number = %d\n" , led_no );
...
void app_main(void)
{
    ...
    // ประกอบ command, ข้อความช่วยเหลือ และ callback function
    const esp_console_cmd_t cmd = {
     ...
     const esp_console_cmd_t led = {
        .command = "led",
        .help = "Control led on esp32\n Usage: led <pin-no> <on|off> ",
        .func = &led_console, 
    };
